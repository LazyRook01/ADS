...
if visualization_type == "Bar Plot":
    ...
    ax = sns.lineplot(x='Date', hue="Country/Region", y="Confirmed", data=df_selected_countries)
    ax.set_xlabel('Date')
    ax.set_ylabel('Confirmed Cases')
    ax.set_title('COVID-19 Confirmed Cases by Country/Region (Line Plot)')
    ax.legend(title='Country/Region', loc='upper left')
    # Display the line plot in Streamlit
    st.pyplot(ax.figure)
elif visualization_type == "Scatter Plot":
    # Create the scatter plot
    ax = sns.scatterplot(x='Long', y='Lat', hue="Confirmed", size="Deaths", data=df_selected_countries)
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Latitude')
    ax.set_title('COVID-19 Confirmed Cases and Deaths by Location (Scatter Plot)')
    ax.legend(title='Confirmed Cases', loc='upper right')
